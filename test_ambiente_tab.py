...
def setup_tkinter():
    ...
    root = tk.Tk()
    tab_instance = ambiente_tab(root)
    yield root, tab_instance
    root.destroy()

def test_ambiente_tab_widgets(setup_tkinter):
    ...
    assert label.cget('text') == 'Feitos Ambientais'
    ...
    with patch.object(tab_instance, 'display_activities', autospec=True) as mock_display_activities:
        tab_instance.show()
        mock_display_activities.assert_called_once_with('ambiente')
